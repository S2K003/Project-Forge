...
| Forge pipeline (end to end, deterministic) | `ForgePipelineTests` — the recorded reference implementation compiles against the curated reference set, passes the banned-API audit, and **the sandbox subprocess actually runs** the canonical acceptance suite; every criterion maps to a passing test. A separate test feeds a deliberately non-idempotent implementation and asserts canonical **AC-2 fails** and `RequirementSatisfied` is false |
| Demo Mode integrity | `CustomerHubJourneyTests` — every `JourneyStepKind` present and ordered, the recorded audit permits execution, every acceptance criterion is satisfied by an executed test, every recorded AI interaction is flagged `Simulated`, health weights sum to 1 |

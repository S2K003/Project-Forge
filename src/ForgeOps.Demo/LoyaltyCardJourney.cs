...
public static class LoyaltyCardJourney
{
    ...
    private static JourneyStep Implementation() => new()
    {
        ...
        Payload = new StepPayload
        {
            ...
            {
                Summary = "A compact dark loyalty card: points balance, tier badge with progress to the next tier, and the last three activity rows.",
            ...
            AiInteraction = Recorded("webcomp.v1", 15200, 0.7)
        }
    };
        ...
        Payload = new StepPayload
        {
            AiInteraction = Recorded("codereview.v1", 4800, 0.71) with
            {
                Decision = new ForgeOps.Contracts.Ai.HumanDecision
                {
                    ...
                    Reason = "Audit clean. Accepting the two low-severity notes as follow-ups. Render it."
                }
            },
    ...
    private static JourneyStep Merge() => new()
    {
        ...
        Title = "Merge",
        Caption = "Audit is green and a human approved. The component ships.",
        SimulatedThinkingMs = 900,
        Payload = new StepPayload
        {
            Gates =
            [
                ...
                Gate("Behavioural checks", GateStatus.Passed, 1, ["4 / 4 in the sandboxed iframe"])
            ],
            PullRequest = new PullRequestSummary
            {
                Number = 148, Title = "Add loyalty status card component", Branch = "feature/loyalty-card",
                FilesChanged = 1, Additions = 132, Deletions = 0
            },
            Notes = ["PR #148 merged. Component rendered and checked; two a11y notes tracked as follow-ups."]
        }
    };
    ...
    private static JourneyStep Telemetry() => new()
    {
        ...
        Payload = new StepPayload
        {
            Telemetry =
            [
                new TelemetrySample { Metric = "forgeops_ai_request_duration p95", Value = "15.2 s", Detail = "webcomp.v1 via AI Bridge" },
                new TelemetrySample { Metric = "forgeops_forge_repair_rounds", Value = "0" },
                new TelemetrySample { Metric = "forgeops_ui_checks_passed_ratio", Value = "4 / 4" },
    ...
    private static JourneyStep Health() => new()
    {
        ...
        Payload = new StepPayload
        {
            Health = new EngineeringHealth
            {
                ...
                Reasons =
                [
                    new HealthReason { Kind = ReasonKind.Pass, Text = "4 / 4 behavioural checks passed in the sandbox" },
                    ...
                    new HealthReason { Kind = ReasonKind.Warn, Text = "2 low-severity accessibility notes from AI review are open" },
